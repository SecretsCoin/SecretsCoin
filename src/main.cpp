...
unsigned int nStakeMinAge = 60 * 60 * 3;	// minimum age for coin age: 3h
...
int64 nChainStartTime = 1449979200;
...
const string strMessageMagic = "SecretsCoin Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // SecretsCoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, uint256 prevHash)
{
     ...
     if(nHeight == 1)
     {
       nSubsidy = 10000000 * COIN;	// 10 million coins, that all pow coins
...
//const int YEARLY_BLOCKCOUNT = 0;	// 365 * 2880
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in SecretsCoin
    bool fStrictPayToScriptHash = true; // Always active in SecretsCoin
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "SecretsCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "SecretsCoin";
        CTransaction txNew;
        ...
        assert(block.hashMerkleRoot == uint256("0xff884ca83d0c50f62c9476f156fc41afeec19331689a082d93b9fc527dc7e857"));
...
unsigned char pchMessageStart[4] = { 0xec, 0xbf, 0xca, 0xbd };
